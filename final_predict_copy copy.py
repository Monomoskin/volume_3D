...
# --- 2. Procesar cada par de imágenes (TOP y SIDE) ---
def process_sample_pair(predictor, metadata, sample_key, input_image_dir, output_base_dir, category_names, predicted_attributes):
    ...
    if has_side:
        ...
        cv2.imwrite(
            os.path.join(output_base_dir, f"{sample_key}_SIDE_predicted.jpg"),
    ...
    if defective_region_id is not None:
        keep = torch.ones(len(inst_top), dtype=torch.bool, device=inst_top.pred_classes.device)
        for i in range(len(inst_top)):
            cls = inst_top.pred_classes[i].item()
            score = inst_top.scores[i].item()
            if cls == defective_region_id:
                if score < 0.1:  # Threshold bajo SOLO para defectos
                    keep[i] = False
            else:
                if score < 0.7:  # Threshold alto para el resto
                    keep[i] = False
        inst_top = inst_top[keep]

    # Depuración: ver qué detectó
    print(f"[DEBUG {sample_key}] Total detecciones tras threshold: {len(inst_top)}")
    for j in range(len(inst_top)):
        cls_id = inst_top.pred_classes[j].item()
        cls_name = category_names[cls_id]
        scr = inst_top.scores[j].item()
        print(f"  -> Clase: {cls_name} | Score: {scr:.3f}")
    ...
    for i in range(len(inst_top)):
        ...
        if defective_region_id is not None:
            ...
            for j in range(len(inst_top)):
                ...
                if np.sum(defect_mask) == 0:
                    continue
                overlap_ratio = np.sum(defect_mask & cell_mask) / np.sum(defect_mask)
        ...
        # --------------------------------------------------
        # 🔥 CALIDAD: SOLO BASADA EN % DEFECTUOSO (100 - defect %)
        # --------------------------------------------------
        final_quality_percent = 100.0 - defective_percent
        ...
        # ---- ATRIBUTOS RESTANTES (solo species y stage si existen)
        # --------------------------------------------------
        species = stage = None
        ...
        predicted_attributes[cell_key] = {
            ...
            "final_quality_percent": final_quality_percent
        }
    ...
    # --------------------------------------------------
    # 4. VISUALIZACIÓN TOP (defectos en rojo visible)
    # --------------------------------------------------
    ...
    for i in range(len(inst_top)):
        ...
        if class_name == "defective_region":
            v.draw_binary_mask(mask, alpha=0.7, color=(1.0, 0.0, 0.0))  # Rojo en rango 0-1
        else:
    ...
    cv2.imwrite(
        os.path.join(output_base_dir, f"{sample_key}_TOP_predicted.jpg"),
        v.output.get_image()[:, :, ::-1]
    )
...
def consolidate_results(output_base_dir, real_volume_map, predicted_attributes):
    ...
    for cell_key, attr in predicted_attributes.items():
        # Manejar volume_ml: si es None → np.nan
        volume_ml_raw = attr.get("volume_ml")
        volume_ml = np.nan if volume_ml_raw is None else float(volume_ml_raw)

        # Mapear volumen real
        sample_key_only = cell_key.rsplit("_cell", 1)[0]
        real_volume_raw = real_volume_map.get(sample_key_only)
        real_volume = np.nan if real_volume_raw is None else float(real_volume_raw)

        # Calcular error y precisión solo si ambos son números válidos
        if np.isfinite(volume_ml) and np.isfinite(real_volume) and real_volume > 0:
    ...
    if not all_rows:
        print("[WARN] No hay datos para consolidar.")
        return

    df = pd.DataFrame(all_rows)
    master_csv_path = os.path.join(output_base_dir, 'all_volumes_summary.xlsx')
    df.to_excel(master_csv_path, index=False)
    print(f"\n✅ CONSOLIDACIÓN EXITOSA: {len(all_rows)} registros guardados en")
    print(f"   {master_csv_path}")

    # Estadísticas finales
    valid = df.dropna(subset=['volume_ml', 'real_volume_ml', 'error_abs_ml', 'precision_percent'])
    if not valid.empty:
        mean_error = valid['error_abs_ml'].mean()
        mean_precision = valid['precision_percent'].mean()
        print(f"   → Error absoluto promedio: {mean_error:.4f} mL")
        print(f"   → Precisión promedio: {mean_precision:.2f}%")
    else:
        print("   → No hay muestras con volumen real y predicho para calcular precisión.")
